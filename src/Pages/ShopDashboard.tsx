...
export default function ShopDashboard() {
  ...
  useEffect(() => {
    const getData = () => {
      let rowsString = localStorage.getItem("ShopArray") || "No Data in the DataBase";
  ...
  const HandelAdd = (values: List) => {
    let PreviousData = localStorage.getItem("ShopArray")
    if (PreviousData === null || PreviousData.trim() === "") {
      ...
      localStorage.setItem("ShopArray", UpdatedData)
  ...
  return (
    <div>
      ...
      {/* UPdate From */}
      {
        UpdateFrom === true && <div >
          <div>
            <Modal open={open} onClose={handleClose} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
              <Box sx={style}>
                <UpdateShopForm onUpdate={HandelUpdate} Shop={Update} />
              </Box>
            </Modal>
          </div>
        </div>
      ...
      {/* Shop table */}
      {rows !== null && <ShopTable rows={rows} onDelete={HandelDelete} onUpdate={HandelUpdateData} onProduct={HandelProduct} />}
    </div>
  );
}
